...
def calculate_pnl(coin_data):
    if coin_data['total_bought'] == 0:
        return 0
    return (coin_data['total_sold_eth'] - coin_data['total_bought_eth']) / coin_data['total_bought_eth'] * 100
...
def calculate_statistics(transactions: List[SwapTransactionResult]) -> StatisticsResult:
    ...
    median_trade_times = {coin: np.median([time.timestamp() for time in data['times']]).astype(datetime) for coin, data in coins_data.items()}
